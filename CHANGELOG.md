...
- docs(feedback): round-5 external review (6 items — graph depth / procedural memory / ecosystem SDK / production governance / scaling & ops / engineering) dispositioned at the doc layer; src/ untouched. README gains "Three boundaries" under Positioning (README.md:339–343) and "Ecosystem & integration paths" (README.md:357–363), both mirrored in README.zh-CN.md (:193–197, :199–205). Verdicts, three-state with evidence: **过时 ×3** — "no Python SDK / no license / no contributing guide": `sdk/python/memoryengine` shipped at `7694b3f` (2026-09-17), `LICENSE` + `CONTRIBUTING.md` in the tree since the first public snapshot `9daa9c5` (2026-09-16); "no CI": `.github/workflows/ci.yml` since `ab9f168`, first green run 36; "no scaling story / no observability": S0–S4 ladder public at ROADMAP.md §1, `GET /v1/metrics` added this cycle (`22ab945`). **半真 ×2** — production governance: tenant fields + RLS are pre-staged (README.md:81) but quota isolation and privacy boundaries remain the named S2 prerequisites (README.md:303) and there is no per-client 429 / circuit breaker (only 503+`retryable`) — a real gap with a stated boundary, not an absent one; procedural memory: typing ships on both sides of the wire (explicit `memory_type` accepted at retain, `src/memory_engine/api_core.py:48`; recall filter, README.md:75) with live distribution episodic 23,549 / procedural 14,987 / semantic 921 measured 2026-09-21 — but the heuristic classifier carries no labeled-precision measurement and procedural entries are stored, not executed (host-side semantics). **真缺 ×3, registered not papered over** — MCP server (decision item `7dd74d9f`, 2026-09-21, build in flight), TS SDK (on demand, no signal yet, deliberately unscheduled), import API (export ships as `GET /v1/export`, re-import does not). Graph-depth verdict: the recall route stays one-hop pull-back over 2,463 entities / 7,975 semantic edges (2026-09-18 snapshot; 3,989 / 13,788 live 2026-09-21) plus browsable two-hop neighbors — multi-hop reasoning, entity disambiguation, and temporal edge invalidation are roadmap items, self-counted from the live DB: **0 of 13,788 edges has `invalid_at` set**, which is exactly why this batch claims no Zep/Graphiti parity.
